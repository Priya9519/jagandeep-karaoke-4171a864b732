...
/**
 * Created by sierra1 on 10/6/17.
 */

public class IGOTVExoplayer {

    private static IGOTVExoplayer mInstance = null;
    private static final DefaultBandwidthMeter BANDWIDTH_METER = new DefaultBandwidthMeter();
    private String TAG = "OminaAudioPlayer";
    ...
    public static IGOTVExoplayer getSharedInstance(Context mContext){
        if(mInstance == null)
        {
            mInstance = new IGOTVExoplayer(mContext);
        }
        return mInstance;
    }

    private IGOTVExoplayer(Context mContext){
...
//            ArrayList<String> urlss = readURLs(urlToPlay);
            mp4VideoUri =Uri.parse(urlToPlay);//urlss.get(0));
            videoSource = new ExtractorMediaSource(mp4VideoUri, dataSourceFactory, new DefaultExtractorsFactory(),
                    null, null);
        }

        // new ExtractorMediaSource(mp4VideoUri, dataSourceFactory, new DefaultExtractorsFactory(),
        //null, null);//new HlsMediaSource(mp4VideoUri, dataSourceFactory, 1, null, null);
//        final LoopingMediaSource loopingSource = new LoopingMediaSource(videoSource);
